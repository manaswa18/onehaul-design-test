...
import { Add, DocIcon, HelpIcon, NotificationIcon, FilterIcon, Search } from '@/icons';
import './carriers.css';
...
interface Carrier {
  id: string;
  // TODO: add fields when wiring real data
}

// ─── Mock data ────────────────────────────────────────────────────────────────

const MOCK_ACTIVE: Carrier[]   = [];
const MOCK_INACTIVE: Carrier[] = [];

// ─── Columns (scaffold — add via /table-designer) ─────────────────────────────
...
export default function CarriersPage() {
  ...
  const filteredActive = MOCK_ACTIVE.filter(c => {
    if (!searchQuery) return true;
    const q = searchQuery.toLowerCase();
    return c.id.toLowerCase().includes(q);
  });

  const filteredInactive = MOCK_INACTIVE.filter(c => {
    if (!searchQuery) return true;
    const q = searchQuery.toLowerCase();
    return c.id.toLowerCase().includes(q);
  });

  function renderTabContent(data: Carrier[]) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div className="carriers-toolbar">
          ...
          <div className="carriers-toolbar-right">
            <Input
              floated={false}
        ...
        <Table data={data} columns={COLUMNS} options={{}} />
      </div>
    );
  }
  ...
  const tabItems = [
    {
      key: 'all-carriers',
      label: 'All Carriers',
      children: renderTabContent(filteredActive),
    },
    {
      key: 'inactive',
      label: 'Inactive Carriers',
      children: renderTabContent(filteredInactive),
    },
  ];
      ...
      >
        <div style={{ flex: 1, overflowY: 'auto', padding: '32px 40px 40px', display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div className="carriers-header">
            <div className="carriers-header-left">
              ...
              <Text variant="body" size="sm" style={{ color: 'var(--theme-color-grey-50)' }}>
                Manage shipping lines, airlines, and transport providers used across bookings and shipments.
              </Text>
            </div>
            <div className="carriers-header-actions">
              <Button variant="secondary" size="md">Import Carriers</Button>
              <Button variant="primary" size="md" icon={<Add width={14} height={14} />}>Add Carrier</Button>
            </div>
          </div>

          {/* Primary tabs */}
          <Tabs items={tabItems} defaultActiveKey="all-carriers" />
